...
public class Student_PrelabQuiz extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Student_PrelabQuiz frame = new Student_PrelabQuiz(null,null,null);
	 ...
	 * Create the frame.
	 */
	public Student_PrelabQuiz(final UserInfo user_info, final String course_number, final String lab_name) {
		...
		final Quiz prelab_quiz = new Quiz(course_number, lab_name, "prelab");
		...
		btnBack.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				Student_Prelab theory = new Student_Prelab(user_info, course_number, lab_name);
		...
		JLabel lblNewLabel = new JLabel("Prelab Quiz");
		lblNewLabel.setFont(new java.awt.Font("Tahoma", java.awt.Font.BOLD, 24));
		lblNewLabel.setBounds(10, 11, 168, 34);
		...
		if(prelab_quiz.get_number_questions() > 12){
			...
			questionPanel.setPreferredSize(new Dimension(questionPanel.getWidth(), questionPanel.getHeight()+113+(prelab_quiz.get_number_questions() * 20)));
		}
		...
		JLabel[] question_labels = new JLabel[prelab_quiz.get_number_questions()];
		final JTextField[] answer_fields = new JTextField[prelab_quiz.get_number_questions()];
		...
		for(int i = 1; i < prelab_quiz.get_number_questions(); i++){
		   ...
		   try (Scanner scanner =  new Scanner(path, ENCODING.name())){
		     ...
		     while (scanner.hasNextLine()){
		       ...
		       if(temp.contains(lab_name)){
		    	   while(!temp.contains("Prelab Questions Path: ")){
		    		   temp=scanner.nextLine();
		    	   }
		    	   pretest_PDF = temp.split("Prelab Questions Path: ")[1];
		    	   break;
		       }
		     }      
		     scanner.close();
		...
		btnNext.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				for(int i = 0; i < prelab_quiz.get_number_questions(); i++){
					
					if(answer_fields[i].getText().isEmpty() || !answer_fields[i].getText().replaceAll("[a-zA-Z]", "").isEmpty()){
						success = false;
				...
				if(success){
					...
					System.out.println(prelab_quiz.check_answers(student_answers) + " out of " + prelab_quiz.get_number_questions());						
					
					Lab_Part_Description next = new Lab_Part_Description(user_info, course_number, lab_name, "1");
